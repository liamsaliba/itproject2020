...
const sendChangeNotification = (email, user, changeItem) => {
  if (!email) {
    return;
  }
  ...
  // Read the pre-compiled message
  fs.readFile("./emailbot/samples/change-noti.txt", "utf-8", (err, data) => {
    if (err) return;
    ...
    const mailOptions = {
      from: process.env.EMAIL_ADDRESS,
      to: email,
      subject: "Camel Pages: Change of details",
      text: data,
    };
    transporter.sendMail(mailOptions, (err, _info) => {
      if (err) {
...
module.exports = { sendGreeting, sendChangeNotification };
